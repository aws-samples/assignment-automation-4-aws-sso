...
- Python requirements from main requirements.txt file 
- At least 2 accounts: 
    - Organization Master account 
    - Deployment and Execution account
## Deployment notes

1. Modify cdk.context.json with appropriate accounts information as well as variables and commit changes. At the bare minimum following 4 parameters needs to be changed:
...
**Make sure to commit these changes ot the local repository, or this changes will not propagate to AWS CodeCommit**
...
1. Now all manual deployments steps are completed. CodePipelines will install everything else automatically. It's possible to track the progress from the deployment account CodePipeline.
## Usage
